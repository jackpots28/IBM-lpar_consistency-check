...
output_log = "/home/<USERID>/move_me/git_projects/lpar_consistency_check/run_output.log"
output_log = "/home/<USERID>/move_me/git_projects/lpar_consistency_check/run_output.log" if os.path.isfile(output_log) else {logging.error("File does not exist!"), sys.exit(20)}
open(output_log, "w").close()

# Test if managed_systems file is writable to
managed_sys_list = "/home/<USERID>/move_me/git_projects/lpar_consistency_check/managed_systems.log"
managed_sys_list = "/home/<USERID>/move_me/git_projects/lpar_consistency_check/managed_systems.log" if os.path.isfile(managed_sys_list) else {logging.error("File does not exist!"), sys.exit(20)}

# Setup some creds for Paramiko to use - NOTE, these strings will need to change depending on who is running this
host_list_path = "/home/<USERID>/lists/hmc.list"
account = "<USERID>"
rsa_key = "/home/" + account + "/.ssh/id_rsa"

host_list_path = "/home/<USERID>/lists/hmc.list" if (os.path.isfile(host_list_path)) else {logging.error("File does not exist!"), sys.exit(20)}
account="<USERID>" if (os.path.isdir("/home/"+account)) else {logging.error("User does not exist!"), sys.exit(30)}
...
class retrieve_lpar_info():
    ...
    if switch:
        frame_grouping_desc = run_command(remote_host, account, pkey, "lssyscfg -r sys -F name description")
    ...
    for lr_frame, oc_frame in sister_pairs.itertuples(index=False):
        ...
        # We dont care if they match, just say its good and continue one
        if lr_test_df.loc[:, lr_test_df.columns != 'lpar_name'].equals(oc_test_df.loc[:, oc_test_df.columns != 'lpar_name']):
            ...
            print(diff_df.to_string(index=False), end='\n\n')
            print("-" * 200)
            with open(output_log, "a") as logging_file:
                logging_file.write(f"{diff_df.to_string(index=False)}" + "\n\n")
                logging_file.write("-" * 200 + "\n")
